...
public:
  RuntimeManager(std::unique_ptr<jsi::Runtime>&& runtime,
                 ...
                 std::shared_ptr<Scheduler> scheduler): runtime(std::move(runtime)), errorHandler(errorHandler), scheduler(scheduler), workletsCache(std::make_unique<WorkletsCache>()), storeUserData(std::make_shared<StaticStoreUser>()) { }
